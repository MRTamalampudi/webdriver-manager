...
pub trait WebdriverManager {
        ...
        if WINDOWS.is(self.get_os()) {
            ...
            for env in envs {
                ...
                if env.eq(ENV_PROGRAM_FILES) && env_value.contains(ENV_X86) {
                    ...
                    // architectures (since the webdriver-manager in Windows is compiled as x32 binary)
    ...
    fn setup(&mut self) -> Result<PathBuf, Error> {
        ...
        if self.get_driver_version().is_empty() || (self.is_nightly(self.get_driver_version())) {
    ...
    fn set_offline(&mut self, offline: bool) {
        if offline {
            self.get_logger()
                .debug("Using Webdriver Manager in offline mode");
...
pub fn get_manager_by_browser(browser_name: String) -> Result<Box<dyn WebdriverManager>, Error> {
...
pub fn get_manager_by_driver(driver_name: String) -> Result<Box<dyn WebdriverManager>, Error> {
